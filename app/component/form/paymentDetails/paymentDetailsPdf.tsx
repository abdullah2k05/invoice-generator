...
export const PaymentDetailsPdf: React.FC<PaymentDetailsPdfProps & { template?: PdfTemplate }> = ({
  ...
  const paymentTitleStyle: any = {
    ...pdfTypography.paymentTitle,
    color: tc?.paymentTitle || pdfTypography.paymentTitle.color,
    fontSize: template?.fontSizes.title ?? 11,
  };
          ...
          <View style={{ flexDirection: "column", gap: 5 }}>
            {bankName && (
              <View style={pdfUtils.flexRowItemCenter}>
                ...
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 44.5 }}>{bankName}</Text>
              </View>
            )}
            {accountNumber && (
              <View style={pdfUtils.flexRowItemCenter}>
                <Text style={paymentTitleStyle}>Account Number</Text>
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 14 }}>{accountNumber}</Text>
              </View>
            )}
            {accountName && (
              <View style={pdfUtils.flexRowItemCenter}>
                <Text style={paymentTitleStyle}>Account Name</Text>
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 26 }}>{accountName}</Text>
              </View>
            )}
            {swiftCode && (
              <View style={pdfUtils.flexRowItemCenter}>
                <Text style={paymentTitleStyle}>Swift Code</Text>
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 45 }}>{swiftCode}</Text>
              </View>
            )}
            {ifscCode && (
              <View style={pdfUtils.flexRowItemCenter}>
                <Text style={paymentTitleStyle}>IFSC Code</Text>
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 48 }}>{ifscCode}</Text>
              </View>
            )}
            {routingCode && (
              <View style={pdfUtils.flexRowItemCenter}>
                <Text style={paymentTitleStyle}>Routing Code</Text>
                <Text style={{ flex: 1, ...itemDescStyle, paddingLeft: 32 }}>{routingCode}</Text>
              </View>
            )}
          </View>
        </View>
